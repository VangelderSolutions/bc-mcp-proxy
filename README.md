...
- **Working with several companies** (`BC_ALLOW_COMPANY_SWITCH=1` / `--AllowCompanySwitch`, off by default). The Business Central MCP server binds a session to one company, so an installation normally serves the configured company only. With the setting on, every tool gets an optional `company` argument and a `bc_list_companies` tool lists the companies the signed-in user can open (standard `companies` API, user's own token). A call with a `company` runs on a separate upstream session for that company, opened on first use; the argument is never forwarded to Business Central. Permission sets are assigned per company, so a company the user lacks rights in answers with the usual permission note. Note that the optional tool hiding judges the configured company only; leave it off when users work across companies with very different rights.
