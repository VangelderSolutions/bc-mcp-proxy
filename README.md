...
- **Hide tools the user may not use** (`BC_HIDE_UNAUTHORIZED_TOOLS=1` / `--HideUnauthorizedTools`, off by default). Static tool mode only: after each connect the proxy reads one record from every `List…_PAG…` tool and hides the tools for pages Business Central refuses, together with their `Create…`/`ListUpdate…`/`Delete…`/bound-action siblings, then pushes `tools/list_changed`. Timeouts, rate limits and structural errors never hide anything, a live denial hides its page immediately, and Business Central still enforces every call. It costs one call per API page per connect (visible in `RT0054` telemetry), so leave it off on large configurations unless the tool list confuses users. With the companion app **MCP Guard** by Vangelder Solutions (a paid Business Central app, licensed per user; see *BC MCP Enterprise* below) installed and its `effectivePermissions` page in the MCP configuration, the proxy makes one call instead and also hides `Create…`/`ListUpdate…`/`Delete…` tools individually where the user lacks that table permission; pages that read a second table in code are still caught by the live denial. Business Central refuses that page to users without an MCP Guard licence; for them the proxy probes as above.
- **Working with several companies** (`BC_ALLOW_COMPANY_SWITCH=1` / `--AllowCompanySwitch`, off by default). The Business Central MCP server binds a session to one company, so an installation normally serves the configured company only. With the setting on, every tool gets an optional `company` argument and a `bc_list_companies` tool lists the companies of the environment (standard `companies` API with the user's own token; it lists every company, permission or not). A call with a `company` runs on a separate upstream session for that company, opened on first use; the argument is never forwarded to Business Central. Permission sets are assigned per company, so a company the user lacks rights in answers with the usual permission note. Note that the optional tool hiding judges the configured company only; leave it off when users work across companies with very different rights. `BC_ALLOWED_COMPANIES="A;B"` / `--AllowedCompanies` narrows the switch (and the listing) to those companies plus the configured one; like the switch itself this is a convenience limit, not a security boundary.
...
A hook can also hand the connection **several environments** (0.12): `dataclasses.replace(config, environments=(EnvironmentTarget(name=..., company=..., configuration_name=..., allowed_companies=...), ...))`, with the configured environment first. Every tool then gets an optional `environment` argument next to `company`, `bc_list_companies` describes each environment with its own companies and default, and a call routed elsewhere runs on its own upstream session, built from that environment's company, configuration and allowed companies (the environment travels in the session: a header on the v28 endpoint, the path on the legacy one). `environment_note` adds one sentence to the listing, for the embedding package to say why the list is what it is - which environments were left out and what decides that - because the proxy is handed the result, not the rules. It requires `allow_company_switch` and has no environment variable or command-line flag: deciding which environments a user may reach is what the embedding package's own rules are for. Two limits to know: `tools/list`, the pre-warm and the optional tool hiding all follow the **configured** environment, so give the environments configurations that expose the same tools — a tool that exists only in one of them fails in the other — and every environment is reached with the same signed-in user, so Business Central decides per environment what they may do.

What a hook decides from can change while the connection is up: a user's permissions, or the settings an administrator keeps in Business Central. With `allow_live_reconfigure` (0.13, off by default) the hook may hand in a later result instead of waiting for the next start. Define `attach(context)` on the hook object; the proxy calls it with a `PrepareContext` before running the hook, and `await context.reconfigure(new_config)` then moves the connection onto the new `allowed_companies`, `environments` and `environment_note`. `bc_list_companies` reports them from the next call on, and a company that has become allowed can be routed to at once. Only those three fields are taken: the upstream sessions, the per-company sessions and the tools cache are left exactly as they are, and a different default environment or company is refused with a warning because that needs the connection rebuilt. A plain callable hook has no `attach` and is unaffected.
...
| Allow Company Switch | `--AllowCompanySwitch` / `--NoAllowCompanySwitch` | `BC_ALLOW_COMPANY_SWITCH` | off — adds a `company` argument to every tool and the `bc_list_companies` tool |
